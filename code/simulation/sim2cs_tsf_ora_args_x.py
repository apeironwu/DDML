    ...
    for rnd_gen in (2023 + np.array(range(n_rnp_gen))): 
        for rnd_ds in (2023 + np.array(range(n_rnp_ds))):
            ...
            try:
                ...
                vec_beta_est_cen = np.zeros(K_fold)
                vec_beta_cs_est_cen = np.zeros(K_fold)
                ...
                for splt in range(K_fold):
                    ...
                    for j_cen in [0]:
                        ...
                        for j in range(K):
                            ...
                            mat_U_slope_cs[:, j] = mat_U_slope_cs[:, j] * (
                                mat_X_cen[:, j_cen] - mat_X_cen[:, j]
                            )
                            mat_U_slope_cs[:, j] = np.exp(-0.5 * mat_U_slope_cs[:, j])
                            mat_U_slope_cs[:, j] = mat_U_slope_cs[:, j] * mat_U_slope[:, j]
                        ...
                        vec_beta_est_cen[splt] = beta_est_cen

                        beta_est_cen_cs = beta_est_ini + S / U_slope_cs
                        vec_beta_cs_est_cen[splt] = beta_est_cen_cs
                ...
                vec_beta_est_iter[i_iter] = beta_est_cs
