...
for rnd_np in (128 + np.array(range(n_rnp))): 
    for rnd_ds in (128 + np.array(range(n_rds))):
        ...
        # print(rnd_np, beta_est, beta_est_ini, sep=", ")


        ## iteration
        while i_iter < n_iter:

            ## updating data splitting
            idx_est = np.array(list(set(random.sample(range(n), n_est)))) ## random splitting
            idx_nui = np.array(list(set(range(n)) - set(idx_est)))
            ...
            for j in range(K):
                ...
                ## updating estimation of mu
                model_mu = RandomForestRegressor(n_estimators=n)
                model_mu.fit(mat_X_nui, vec_D_nui)

                list_mu_est[j] = model_mu
        ...
        # print("M1:       ", vec_beta_est_iter[0])
        # print("M2:       ", vec_beta_est_iter[1])
        # print("A10:      ", vec_beta_est_iter[:10].mean())
        # print("M10:      ", np.median(vec_beta_est_iter[:10]))
        # print("A20:      ", vec_beta_est_iter[:20].mean())
        # print("M20:      ", np.median(vec_beta_est_iter[:20]))
        # print("A50:      ", vec_beta_est_iter.mean())
        # print("M50:      ", vec_beta_est_iter.median())
        ...
        print(
            ...
            vec_beta_est_iter[0],
            vec_beta_est_iter[1], 
            np.mean(vec_beta_est_iter[:10]),
            np.median(vec_beta_est_iter[:10]),
            # np.mean(vec_beta_est_iter[:20]),
            # np.median(vec_beta_est_iter[:20]),
            # np.mean(vec_beta_est_iter[:50]),
            # np.median(vec_beta_est_iter[:50]),
            np.any(np.isnan(vec_beta_est_iter)),
            sep=","
        )
