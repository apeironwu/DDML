...
def main(argv): 
    ...
    vec_U_var_est = np.zeros(K)
    vec_U_var_ora_est = np.zeros(K)

    vec_V_var_est = np.zeros(K)
    vec_V_var_ora_est = np.zeros(K)
        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                for j in range(K): 
                    ...
                    vec_U_var_est[j] = np.var(vec_Y_diff - vec_D_est * beta_est_ini)
                    vec_U_var_ora_est[j] = np.var(vec_Y_ora_diff - vec_D_est * beta)

                    vec_V_var_est[j] = np.var(vec_D_diff)
                    vec_V_var_ora_est[j] = np.var(vec_D_ora_diff)
                    
                    # print("Var(U):     ", np.var(vec_Y_ora_diff - vec_D_est * beta))
                    # print("Var(Est U): ", np.var(vec_Y_diff - vec_D_est * beta))

                    # print("      ", "Var(V):     ", np.var(vec_D_ora_diff))
                    # print("      ", "Var(Est V): ", np.var(vec_D_diff))
                ...
                print("var(U)_est:   ", np.mean(vec_U_var_est), "(", np.std(vec_U_var_est), ")")
                print("var(V)_est:   ", np.mean(vec_V_var_est), "(", np.std(vec_V_var_est), ")")
