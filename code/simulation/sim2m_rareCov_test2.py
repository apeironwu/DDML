...
## nonlinear case
def fun_mu(X, j, ip = 2.0):
    p = len(X)
    jp_step = 5  ## default
    jp = np.mod(j + jp_step, p) 
    out = 10 * fun_sigm(2 * (X[j] - ip)) + X[jp]   ## rareCov 
    return out
    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                for j in range(K): 
                    mat_D[:, j] = np.array(list(map(lambda X: fun_mu(X, j, mu_ip), arr_X[j]))) 

                    mat_D[:, j] = 10 * (mat_D[:, j] > 5).astype(float) ## binary shift
                    
                    mat_D[:, j] = mat_D[:, j] + mat_V[:, j] ## small noise

                    # print(">> rare prop", np.mean(mat_D[:, j] > 5).astype(float).round(2))
                    # print(">> mean_v: ", np.mean(mat_D[:, j] - np.array(list(map(lambda X: fun_mu(X, j, mu_ip), arr_X[j])))))
                ...
                for j in range(K):
                    ...
                    # vec_Y_gam_diff_ora = vec_Y_est - np.array(list(map(lambda X: fun_gamma(X, j), mat_X_est)))

                    # print(
                    #     ">> vec_Y_gam_diff: ", 
                    #     np.corrcoef(vec_Y_gam_diff, vec_Y_gam_diff_ora).round(5)
                    # )
                    ...
                    # print(
                    #     ">> term1: ", np.mean(vec_Y_gam_diff * vec_D_diff).round(5), 
                    #     ">> term2: ", np.mean(vec_D_est * vec_D_diff).round(5),
                    # )
                ...
                for j in range(K): 
                    ...
                    vec_D_ora_diff = vec_D_est - np.array(list(map(lambda X: fun_mu(X, j, mu_ip), mat_X_est))) 
                ...
                for j_cen in range(K):
                    ...
                    if den_est == "single":
                        ...
                        vec_D_cen_diff_ora = vec_D_cen - np.array(list(map(lambda X: fun_mu(X, j_cen, mu_ip), mat_X_cen)))
                        ...
                        for j in range(K):
                            ...
                            vec_D_loc_diff_ora = vec_D_cen - np.array(list(map(lambda X: fun_mu(X, j, mu_ip), mat_X_cen)))
