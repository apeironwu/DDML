...
def main(argv): 
    ...
    prompt_help = [
        ...
        '--den_est <density estimation method, "double">'
    ]
    ...
    vec_beta_single_est_iter = np.zeros(n_iter)
    ...
    if path_out is None: 
        print(
            ...
            ",".join(["Mc" + str(i + 1) for i in range(n_iter)]),
    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            try:
                ...
                beta_est_single = vec_beta_est_cen[0]  ## single site estimation
                beta_est_ora = np.mean(vec_beta_ora_est_cen)

                vec_beta_est_iter[i_iter] = beta_est
                vec_beta_single_est_iter[i_iter] = beta_est_single
                ...
                while i_iter < n_iter:
                    ...
                    vec_beta_single_est_iter[i_iter] = 0.0
                    ...
                    #     model_gamma_single = RandomForestRegressor(n_estimators=n_rft)
                    #     model_gamma_single.fit(mat_X_nui, vec_Y_nui - vec_D_nui * beta_est_single)

                    #     list_gamma_single_est[j] = model_gamma_single

                    # beta_est_ini = beta_est
                    # beta_est_single_ini = beta_est_single

                    # ## statistics from other sites
                    # vec_s = np.zeros(n_est)
                    # vec_S_est = np.zeros(K)

                    # vec_s_single = np.zeros(n_est)
                    # vec_S_single_est = np.zeros(K)
                    ...
                    #     vec_Y_diff_single = vec_Y_est - list_gamma_single_est[j].predict(mat_X_est)

                    #     vec_s_single = (vec_Y_diff_single - vec_D_est * beta_est_single_ini) * vec_D_diff
                    #     vec_S_single_est[j] = np.mean(vec_s_single)
                    ...
                    # S = np.mean(vec_S_est)
                    # S_single = np.mean(vec_S_single_est)
                    # S_ora = np.mean(vec_S_ora_est)
                ...
                if path_out is None: 
                    print(
                        ...
                        ','.join(str(b.round(5)) for b in vec_beta_single_est_iter), " | ",
