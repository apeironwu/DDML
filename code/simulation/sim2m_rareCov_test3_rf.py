...
## nonlinear case
def fun_mu(X, j, alpha = 2, beta = 2, ip = 2.0):
    p = len(X)
    jp_step = 2  ## default
    j = np.mod(j, p) 
    jp = np.mod(j + jp_step, p) 
    out = alpha * fun_sigm(beta * (X[j] - ip)) + 1.5 * X[jp]   ## rareCov 
    return out
        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                for j in range(K): 
                    ...
                    # mat_D[:, j] = mu_alpha * (mat_D[:, j] > (mu_alpha / 2.0)).astype(float) ## binary shift
                    
                    mat_D[:, j] = mat_D[:, j] + mat_V[:, j] ## small noise

                    # print(">> rare prop", np.mean(mat_D[:, j] > 5).astype(float).round(2))
                    # print(">> mean_v: ", np.mean(mat_D[:, j] - np.array(list(map(lambda X: fun_mu(X, j, mu_alpha, mu_beta, mu_ip), arr_X[j])))))

                    mat_Y[:, j] = mat_D[:, j] * beta \
                        + np.array(list(map(lambda X: fun_gamma(X, j), arr_X[j]))) + mat_U[:, j]

                
                rf_mu = RandomForestRegressor(random_state=12)
                rf_gamma = RandomForestRegressor(random_state=12)
                ...
                grid_search_mu.fit(arr_X[1], mat_D[:, 1])
                ...
                grid_search_gamma.fit(arr_X[1], mat_Y[:, 1] - mat_D[:, 1] * beta)

                print("Best Parameters for Mu:", grid_search_mu.best_params_)
