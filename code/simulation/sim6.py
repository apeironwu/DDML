import numpy as np
import random
from sklearn.ensemble import RandomForestRegressor
import time
import sys, getopt

def fun_sigm(X): 
    out = np.exp(X) / (1 + np.exp(X)) 
    return out

## nonlinear case - return the probability of rare event
def fun_mu(X, j, alpha = 1, beta = 10, ip = 2.0):
    j = 0
    p = len(X)
    jp_step = 1  ## default
    j = np.mod(j, p) 
    jp = np.mod(j + jp_step, p) 
    out = .25 * X[jp] + fun_sigm(X[j]) 
    out = fun_sigm(beta * (out - ip))  ## rareCov 
    return out

def fun_gamma(X, j, vec_gam = [1.0, 2.0]): 
    j = 0
    p = len(X)
    jp_step = 1  ## default
    jp = np.mod(j + jp_step, p)
    out = vec_gam[0] * X[j] + vec_gam[1] * fun_sigm(X[jp])  ## default
    return out


def main(argv): 

    ## default values
    n = 300
    K = 10
    p = 5
    n_rnp = 100
    n_rds = 1
    n_iter = 2
    n_rft = 100
    mu_alpha = 1.0
    mu_beta = 2.0 
    mu_ip = 2.0
    rnp_np_ini = 128
    path_out = None
    bl_cor_X = True
    den_est = "double"
    
    prompt_help = [
        'sim6.py',
        '--n <# sample size>',
        '--K <# of sites>',
        '--p <# of confounders>', 
        '--n_rnp <# replication>',
        '--n_rds <# data splitting>',
        '--n_iter <# iteration>',
        '--n_rft <# random forest trees>',
        '--mu_alpha <shift scale in mu>',
        '--mu_beta <max slope in mu>',
        '--mu_ip <inflection point in mu>',
        '--rnp_np_ini <rnp np initial>',
        '--path_out <output path>',
        '--bl_cor_X <correlated X (True) or not (False)>',
        '--den_est <density estimation method, "single", or "ora_dou">'
    ]

    try:
        opts, args = getopt.getopt(
            argv, "h",
            [
                "n=",
                "K=",
                "p=",
                "n_rnp=",
                "n_rds=", 
                "n_iter=",
                "n_rft=",
                "mu_alpha=",
                "mu_beta=",
                "mu_ip=",
                "rnp_np_ini=",
                "path_out=",
                "bl_cor_X=",
                "den_est="
            ]
        )
    except getopt.GetoptError:
        print("\n    ".join(prompt_help))
        sys.exit(2)

    for opt, arg in opts:
        if opt == '-h':
            print("\n    ".join(prompt_help))
            sys.exit()
        elif opt in ("--K"):
            K = int(arg)
        elif opt in ("--p"):
            p = int(arg)
        elif opt in ("--n"):
            n = int(arg)
        elif opt in ("--n_rnp"):
            n_rnp = int(arg)
        elif opt in ("--n_rds"):
            n_rds = int(arg)
        elif opt in ("--n_iter"):
            n_iter = int(arg)
        elif opt in ("--n_rft"):
            n_rft = int(arg)
        elif opt in ("--mu_alpha"):
            mu_alpha = float(arg)
        elif opt in ("--mu_beta"):
            mu_beta = float(arg)
        elif opt in ("--mu_ip"):
            mu_ip = float(arg)
        elif opt in ("--rnp_np_ini"):
            rnp_np_ini = int(arg)
        elif opt in ("--path_out"):
            path_out = arg
        elif opt in ("--bl_cor_X"):
            bl_cor_X = arg == "True"
        elif opt in ("--den_est"):
            den_est = arg

    while den_est not in ["single", "double", "ora_dou"]:
        sys.exit("den_est must be either 'single', 'double' or 'ora_dou'.")
    
    print('=' * 20, "Parameter Setting", '=' * 20)
    print('>> n: ', n)
    print('>> K: ', K)
    print('>> p: ', p)
    print('>> n_rnp: ', n_rnp)
    print('>> n_rds: ', n_rds)
    print('>> n_iter: ', n_iter)
    print('>> n_rft: ', n_rft)
    print('>> mu_alpha: ', mu_alpha)
    print('>> mu_beta: ', mu_beta)
    print('>> mu_ip: ', mu_ip)
    print('>> rnp_np_ini: ', rnp_np_ini)
    print('>> path_out: ', path_out)
    print('>> bl_cor_X: ', bl_cor_X)
    print('>> den_est: ', den_est)

    time_start = time.time()

    ## parameter setting
    vec_beta_est_iter = np.zeros(n_iter)

    beta = -2 ## default

    # large variance
    psi_u = 4.0   ## default
    psi_v = 1.0   ## default


    if path_out is None: 
        print(
            "rnd_np", "rnd_ds", "Average", "IVWAvg", "Mc1",
            ",".join(["M" + str(i + 1) for i in range(n_iter)]),
            # ",".join(["Mo" + str(i + 1) for i in range(n_iter)]),
            sep=",", 
        )
    else:
        print(
            "rnd_np", "rnd_ds", "Average", "IVWAvg", "Mc1", 
            ",".join(["M" + str(i + 1) for i in range(n_iter)]),
            # ",".join(["Mo" + str(i + 1) for i in range(n_iter)]),
            sep=",", 
            file = open(path_out, "w")
        )

    ## data generation 
    #### randomization
    n_np_succ = 0
    rnd_np = rnp_np_ini

    while n_np_succ < n_rnp:
    # for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        rnd_ds = 128
        n_ds_succ = 0

        for rnd_ds in (rnd_ds + np.array(range(n_rds))):
            try:
                # print(rnd_np, rnd_ds)
            
                ## set `numpy` random seed
                np.random.seed(rnd_np)
                
                if bl_cor_X is False:
                    #### uncorrelated X
                    arr_X = np.random.randn(K, n, p)
                else:
                    #### correlated X
                    covmat_X = np.fromfunction(lambda i, j: np.power(0.7, np.abs(i - j)), (p, p))
                    arr_X = np.random.multivariate_normal(np.zeros(p), covmat_X, K * n).reshape(K, n, p)

                mat_U = np.random.randn(n, K) * np.sqrt(psi_u)
                psi_u_est = psi_u
                psi_u_inv_est = 1.0 / psi_u_est

                mat_D = np.zeros((n, K))
                mat_Y = np.zeros((n, K))

                mat_gam = np.random.uniform(0.5, 1.0, (2, K))       ## gam setting 7

                for j in range(K): 
                    vec_prob_d = np.array(list(map(lambda X: fun_mu(X, j, mu_alpha, mu_beta, mu_ip), arr_X[j]))) 

                    mat_D[:, j] = np.random.binomial(n = 1, p = vec_prob_d) ## rare event

                    vec_prob_y = mat_D[:, j] * beta \
                        + np.array(list(map(lambda X: fun_gamma(X, j, mat_gam[:, j]), arr_X[j])))

                    vec_prob_y = fun_sigm(vec_prob_y - 2.0)

                    mat_Y[:, j] = np.random.binomial(n = 1, p = vec_prob_y) 

                print(">> Pr(D=1): ", np.mean(mat_D).round(5))
                print(">> Pr(Y=1): ", np.mean(mat_Y, axis=0).round(5))

                exit("Test")

                ## randomly data splitting

                ## set `random` random seed
                random.seed(int(rnd_ds))
                
                # n_est = int(n / 3)
                # idx_est = np.array(list(range(n_est))) ## non-random splitting
                # idx_est = np.array(list(set(random.sample(range(n), n_est)))) ## random splitting
                # idx_nui = np.array(list(set(range(n)) - set(idx_est)))

                K_fold = 3
                idx_ds = np.random.choice(range(K_fold), n, replace=True)

                i_iter = 0

                ## initial estimation of beta and estimation of nuisance parameter
                mat_beta_est_po_local = np.zeros((K, K_fold))
                mat_beta_est_local = np.zeros((K, K_fold))

                list_gamma_est = list()
                list_mu_est = list()

                for j in range(K):
                    for splt in range(K_fold):
                        idx_nui = np.where(idx_ds != splt)[0]
                        idx_est = np.where(idx_ds == splt)[0]

                        ## training ML model
                        mat_X_nui = arr_X[j][idx_nui, :]
                        vec_D_nui = mat_D[idx_nui, j]
                        vec_Y_nui = mat_Y[idx_nui, j]
                        
                        ## estimating
                        mat_X_est = arr_X[j][idx_est, :]
                        vec_D_est = mat_D[idx_est, j]
                        vec_Y_est = mat_Y[idx_est, j]

                        model_mu = RandomForestRegressor(n_estimators=n_rft)
                        model_mu.fit(mat_X_nui, vec_D_nui)
                        
                        ## estimation of beta based on partialling out score function
                        model_xi = RandomForestRegressor(n_estimators=n_rft)
                        model_xi.fit(mat_X_nui, vec_Y_nui)
                        
                        vec_D_diff = vec_D_est - model_mu.predict(mat_X_est)
                        vec_Y_xi_diff = vec_Y_est - model_xi.predict(mat_X_est)
                        
                        ## first partialling-out estimation
                        beta_est_local = np.mean(vec_Y_xi_diff * vec_D_diff) / np.mean(vec_D_diff * vec_D_diff)
                        mat_beta_est_po_local[j, splt] = beta_est_local

                        for _ in range(1): 
                            model_gamma = RandomForestRegressor(n_estimators=n_rft)
                            model_gamma.fit(mat_X_nui, vec_Y_nui - vec_D_nui * beta_est_local)

                            vec_Y_gam_diff = vec_Y_est - model_gamma.predict(mat_X_est)

                            ## second orthogonal estimation
                            beta_est_local = np.mean(vec_Y_gam_diff * vec_D_diff) / np.mean(vec_D_est * vec_D_diff)

                        model_gamma = RandomForestRegressor(n_estimators=n_rft)
                        model_gamma.fit(mat_X_nui, vec_Y_nui - vec_D_nui * beta_est_local)

                        list_mu_est.append(model_mu)
                        list_gamma_est.append(model_gamma)

                        mat_beta_est_local[j, splt] = beta_est_local

                vec_beta_est_local = np.mean(mat_beta_est_local, axis=1)
                # print(vec_beta_est_local)

                beta_est_ini = np.mean(mat_beta_est_local)

                ## variance estimation ===========================================
                vec_beta_var_local = np.zeros(K)
                
                # for j in range(K): 
                #     var_beta_score2 = 0.0
                #     var_beta_j0 = 0.0

                #     for splt in range(K_fold):
                #         idx_nui = np.where(idx_ds != splt)[0]
                #         idx_est = np.where(idx_ds == splt)[0]

                #         ## training ML model
                #         mat_X_nui = arr_X[j][idx_nui, :]
                #         vec_D_nui = mat_D[idx_nui, j]
                #         vec_Y_nui = mat_Y[idx_nui, j]
                                        
                #         ## estimating
                #         mat_X_est = arr_X[j][idx_est, :]
                #         vec_D_est = mat_D[idx_est, j]
                #         vec_Y_est = mat_Y[idx_est, j]

                #         idx_list = splt + j * K_fold

                #         model_mu = list_mu_est[idx_list]
                #         model_gamma = list_gamma_est[idx_list]

                #         vec_D_diff = vec_D_est - model_mu.predict(mat_X_est)
                #         vec_Y_gam_diff = vec_Y_est - model_gamma.predict(mat_X_est)

                #         var_beta_score2 += np.sum(
                #             (
                #                 (vec_Y_gam_diff - vec_D_est * vec_beta_est_local[j]) * vec_D_diff
                #             ) ** 2
                #         )
                #         var_beta_j0 += np.sum(vec_D_est * vec_D_diff)
                        
                #     var_beta_score2 /= n
                #     var_beta_j0 /= n

                #     vec_beta_var_local[j] = var_beta_score2 / (var_beta_j0 ** 2) / n

                # vec_weight_iv = 1.0 / vec_beta_var_local
                # vec_weight_iv /= np.sum(vec_weight_iv)

                beta_est_ivwa = np.sum(vec_weight_iv * vec_beta_est_local)

                # exit("Test")

                ## statistics from other sites
                vec_S_est = np.zeros(K)

                for j in range(K): 
                    for splt in range(K_fold):
                        # idx_nui = np.where(idx_ds != splt)[0]
                        idx_est = np.where(idx_ds == splt)[0]

                        ## estimating
                        mat_X_est = arr_X[j][idx_est, :]
                        vec_D_est = mat_D[idx_est, j]
                        vec_Y_est = mat_Y[idx_est, j]

                        idx_list = splt + j * K_fold

                        vec_D_diff = vec_D_est - list_mu_est[idx_list].predict(mat_X_est)
                        vec_Y_diff = vec_Y_est - list_gamma_est[idx_list].predict(mat_X_est)
                        
                        vec_s = (vec_Y_diff - vec_D_est * beta_est_ini) * vec_D_diff
                        vec_S_est[j] += np.sum(vec_s)
                    vec_S_est[j] /= n
                S = np.mean(vec_S_est)

                ## operation in the central site
                vec_beta_est_cen = np.zeros(K)

                for j_cen in range(K):
                    mat_U_slope = np.zeros((n, K))

                    for splt in range(K_fold):
                        idx_est = np.where(idx_ds == splt)[0]

                        vec_Y_cen = mat_Y[idx_est, j_cen]
                        vec_D_cen = mat_D[idx_est, j_cen]
                        mat_X_cen = arr_X[j_cen][idx_est,]

                        if den_est == "single":
                            exit("Single density estimation is not implemented in this version.")

                        elif den_est == "double":
                            idx_list_cen = splt + j_cen * K_fold

                            vec_D_cen_est = list_mu_est[idx_list_cen].predict(mat_X_cen)
                            vec_Y_cen_diff = vec_Y_cen - list_gamma_est[idx_list_cen].predict(mat_X_cen)

                            vec_U_cen_est = vec_Y_cen_diff - vec_D_cen * beta_est_ini

                            for j in range(K):
                                idx_list_loc = splt + j * K_fold

                                vec_D_loc_est = list_mu_est[idx_list_loc].predict(mat_X_cen)
                                vec_D_loc_diff = vec_D_cen - vec_D_loc_est
                                vec_Y_loc_diff = vec_Y_cen - list_gamma_est[idx_list_loc].predict(mat_X_cen)
                                
                                vec_U_loc_est = vec_Y_loc_diff - vec_D_cen * beta_est_ini
                                
                                mat_U_slope[idx_est, j] = np.power(vec_U_loc_est, 2) - np.power(vec_U_cen_est, 2)
                                mat_U_slope[idx_est, j] = np.exp(-.5 * psi_u_inv_est * mat_U_slope[idx_est, j]) ## density ratio
                                mat_U_slope[idx_est, j] = mat_U_slope[idx_est, j] \
                                    * ((1 - vec_D_cen) + (2 * vec_D_cen - 1) * vec_D_loc_est + 0.005) \
                                        / ((1 - vec_D_cen) + (2 * vec_D_cen - 1) * vec_D_cen_est + 0.005)

                                mat_U_slope[idx_est, j] = mat_U_slope[idx_est, j] * vec_D_cen * vec_D_loc_diff

                        elif den_est == "ora_dou":
                            exit("Oracle double density estimation is not implemented in this version.")

                    U_slope = np.mean(mat_U_slope)

                    beta_est_cen = beta_est_ini + S / U_slope
                    vec_beta_est_cen[j_cen] = beta_est_cen

                ## final estimation
                beta_est = np.mean(vec_beta_est_cen)

                beta_est_mc1 = vec_beta_est_cen[0]

                print(">> beta_est_mc1: ", beta_est_mc1.round(5))

                vec_beta_est_iter[i_iter] = beta_est
                # vec_beta_ora_est_iter[i_iter] = beta_est_ora

                i_iter += 1

                ## iteration
                while i_iter < n_iter:
                    
                    ## updating estimation of nuisance parameter
                    for j in range(K):
                        for splt in range(K_fold):
                            idx_nui = np.where(idx_ds != splt)[0]
                            idx_est = np.where(idx_ds == splt)[0]

                            idx_list = splt + j * K_fold

                            ## training ML model
                            mat_X_nui = arr_X[j][idx_nui, :]
                            vec_D_nui = mat_D[idx_nui, j]
                            vec_Y_nui = mat_Y[idx_nui, j]
                            
                            ## updating estimation of gamma
                            model_gamma = RandomForestRegressor(n_estimators=n_rft)
                            model_gamma.fit(mat_X_nui, vec_Y_nui - vec_D_nui * beta_est)

                            list_gamma_est[idx_list] = model_gamma

                    beta_est_ini = beta_est

                    ## statistics from other sites
                    vec_S_est = np.zeros(K)

                    for j in range(K): 
                        for splt in range(K_fold):
                            idx_nui = np.where(idx_ds != splt)[0]
                            idx_est = np.where(idx_ds == splt)[0]

                            idx_list = splt + j * K_fold

                            ## training ML model
                            mat_X_nui = arr_X[j][idx_nui, :]
                            vec_D_nui = mat_D[idx_nui, j]
                            vec_Y_nui = mat_Y[idx_nui, j]

                            ## estimating
                            mat_X_est = arr_X[j][idx_est, :]
                            vec_D_est = mat_D[idx_est, j]
                            vec_Y_est = mat_Y[idx_est, j]

                            vec_D_diff = vec_D_est - list_mu_est[idx_list].predict(mat_X_est)
                            vec_Y_diff = vec_Y_est - list_gamma_est[idx_list].predict(mat_X_est)
                            
                            vec_s = (vec_Y_diff - vec_D_est * beta_est_ini) * vec_D_diff
                            vec_S_est[j] += np.sum(vec_s)
                        vec_S_est[j] /= n
                    S = np.mean(vec_S_est)

                    ## operation in the central site
                    vec_beta_est_cen = np.zeros(K)

                    for j_cen in range(K):
                        mat_U_slope = np.zeros((n, K))

                        for splt in range(K_fold):
                            idx_est = np.where(idx_ds == splt)[0]

                            vec_Y_cen = mat_Y[idx_est, j_cen]
                            vec_D_cen = mat_D[idx_est, j_cen]
                            mat_X_cen = arr_X[j_cen][idx_est,]

                            if den_est == "single":
                                exit("Single density estimation is not implemented in this version.")

                            elif den_est == "double":
                                idx_list_cen = splt + j_cen * K_fold

                                vec_D_cen_est = list_mu_est[idx_list_cen].predict(mat_X_cen)
                                vec_Y_cen_diff = vec_Y_cen - list_gamma_est[idx_list_cen].predict(mat_X_cen)

                                vec_U_cen_est = vec_Y_cen_diff - vec_D_cen * beta_est_ini

                                for j in range(K):
                                    idx_list_loc = splt + j * K_fold

                                    vec_D_loc_est = list_mu_est[idx_list_loc].predict(mat_X_cen)
                                    vec_D_loc_diff = vec_D_cen - vec_D_loc_est
                                    vec_Y_loc_diff = vec_Y_cen - list_gamma_est[idx_list_loc].predict(mat_X_cen)

                                    vec_U_loc_est = vec_Y_loc_diff - vec_D_cen * beta_est_ini
                                    
                                    mat_U_slope[idx_est, j] = np.power(vec_U_loc_est, 2) - np.power(vec_U_cen_est, 2)
                                    mat_U_slope[idx_est, j] = np.exp(-.5 * psi_u_inv_est * mat_U_slope[idx_est, j]) ## density ratio
                                    mat_U_slope[idx_est, j] = mat_U_slope[idx_est, j] \
                                        * ((1 - vec_D_cen) + (2 * vec_D_cen - 1) * vec_D_loc_est + 0.005) \
                                            / ((1 - vec_D_cen) + (2 * vec_D_cen - 1) * vec_D_cen_est + 0.005)

                                    mat_U_slope[idx_est, j] = mat_U_slope[idx_est, j] * vec_D_cen * vec_D_loc_diff

                            elif den_est == "ora_dou":
                                exit("Oracle double density estimation is not implemented in this version.")

                        U_slope = np.mean(mat_U_slope)

                        beta_est_cen = beta_est_ini + S / U_slope
                        vec_beta_est_cen[j_cen] = beta_est_cen
                    
                    ## final estimation
                    beta_est = np.mean(vec_beta_est_cen)
                    vec_beta_est_iter[i_iter] = beta_est

                    i_iter += 1

                # print("np rnd:       ", rnd_np)
                # print("ds rnd:       ", rnd_ds)
                # print("Average:      ", np.mean(vec_beta_est_local))
                # print("vec_beta_est: ", vec_beta_est_iter)

                if path_out is None: 
                    print(
                        rnd_np,
                        rnd_ds,
                        np.mean(vec_beta_est_local).round(5), " | ",
                        beta_est_ivwa.round(5), " | ",
                        beta_est_mc1.round(5), " | ",
                        ','.join(str(b.round(5)) for b in vec_beta_est_iter), " | ",
                        # ','.join(str(b.round(5)) for b in vec_beta_ora_est_iter),
                        sep=",",
                    )
                else:
                    print(
                        rnd_np,
                        rnd_ds,
                        np.mean(vec_beta_est_local),
                        beta_est_ivwa.round(5), beta_est_mc1.round(5),
                        ','.join(str(b.round(5)) for b in vec_beta_est_iter),
                        # ','.join(str(b.round(5)) for b in vec_beta_ora_est_iter),
                        sep=",",
                        file=open(path_out, "a")
                    )
                
                n_ds_succ += 1

            except ValueError:
                print("ValueError:  ", "rnp_", rnd_np, " rds_", rnd_ds, sep="")
                continue
        
        if n_ds_succ >= min(3, n_rds):
            n_np_succ += 1
        
        rnd_np += 1

    print("running time: ", time.time() - time_start, " seconds ")

if __name__ == "__main__":
    main(sys.argv[1:])