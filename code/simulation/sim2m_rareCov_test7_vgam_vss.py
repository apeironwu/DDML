    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            # if True:
                ...
                ## sample size setting
                if vss == 1:
                    vec_n = np.random.choice([n, int(n / 2)], K)  ## vss setting 1
                elif vss == 2:
                    vec_n = np.random.randint(int(n / 2), n, K)  ## vss setting 2
                else:
                    exit(">> invalid vss input")
                ...
                if bl_cor_X is False:
                    exit(">> correlated X is not supported in this simulation setting.")

                covmat_X = np.fromfunction(lambda i, j: np.power(0.7, np.abs(i - j)), (p, p))

                for j in range(K): 
                    mat_X = np.random.multivariate_normal(np.zeros(p), covmat_X, vec_n[j]).reshape(vec_n[j], p)
                    vec_U = np.random.randn(vec_n[j]) * np.sqrt(psi_u)

                    # print(list(map(lambda X: fun_gamma(X, j, mat_gam[:, j]), mat_X)))
                ...
                for j in range(K): 
                    vec_D = np.zeros(vec_n[j])
                    vec_Y = np.zeros(vec_n[j])

                    vec_prob_d = np.array(list(map(lambda X: fun_mu(X, j, mu_alpha, mu_beta, mu_ip), ls_X[j]))) 
                    vec_D = np.random.binomial(n = 1, p = vec_prob_d) ## rare event
                    vec_Y = vec_D * beta \
                        + np.array(list(map(lambda X: fun_gamma(X, j, mat_gam[:, j]), ls_X[j]))) + ls_U[j]
                ...
                for j in range(K):
                    ...
                    ls_idx_est.append(idx_est)
                    ls_idx_nui.append(idx_nui)               
                ...
                for j in range(K):
                    idx_nui = ls_idx_nui[j]
                    idx_est = ls_idx_est[j]
                    
                    ## training ML model
                    mat_X_nui = ls_X[j][idx_nui, :]
                    vec_D_nui = ls_D[j][idx_nui]
                    vec_Y_nui = ls_Y[j][idx_nui]
                    
                    ## estimating
                    mat_X_est = ls_X[j][idx_est, :]
                    vec_D_est = ls_D[j][idx_est]
                    vec_Y_est = ls_Y[j][idx_est]
                ...
                for j_cen in range(K):
                    idx_est = ls_idx_est[j_cen]

                    vec_Y_cen = ls_Y[j_cen][idx_est]
                    vec_D_cen = ls_D[j_cen][idx_est]
                    mat_X_cen = ls_X[j_cen][idx_est, ]

                    mat_U_slope = np.zeros((vec_Y_cen.size, K))
