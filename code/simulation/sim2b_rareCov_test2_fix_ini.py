...
def main(argv): 
    ...
    # vec_beta_ini_fix = np.random.rand(n_rnp + 1) * 4 - 3
    # vec_beta_ini_fix = np.random.randn(n_rnp + 1) - 1
    vec_beta_ini_fix = np.random.randn(n_rnp + 1) * 0.2 - 1
    vec_beta_ini_fix = np.sort(vec_beta_ini_fix)
    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                beta_est_ini = vec_beta_ini_fix[rnd_np]
                ...
                if path_out is None: 
                    print(
                        rnd_np,
                        rnd_ds,
                        np.mean(vec_beta_ini_fix[rnd_np]).round(5), " | ",
