    ...
    for rnd_np in (128 + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                ## prediction error
                # vec_mu_var = np.zeros(K)
                # vec_gamma_var = np.zeros(K)
                ...
                for j in range(K):
                    ...
                    ## prediction error
                    # vec_mu_se = np.array(list(map(lambda X: fun_mu(X, j), mat_X_est)))
                    # vec_mu_var[j] = np.var(vec_mu_se)

                    # vec_mu_se = vec_mu_se + vec_D_diff - vec_D_est
                    # vec_mu_se = np.power(vec_mu_se, 2)

                    # vec_mu_mse[j] = np.mean(vec_mu_se)

                    # vec_gamma_se = np.array(list(map(lambda X: fun_gamma(X, j), mat_X_est)))
                    # vec_gamma_var[j] = np.var(vec_gamma_se)

                    # vec_gamma_se = vec_gamma_se + vec_Y_diff - vec_Y_est
                    # vec_gamma_se = np.power(vec_gamma_se, 2)

                    # vec_gamma_mse[j] = np.mean(vec_gamma_se)

                    vec_mu_mse[j] = np.mean(np.power(vec_D_diff, 2))
                ...
                if path_out is None: 
                    ...
                    # print("Var(mu):      ", np.mean(vec_mu_var))
                    print("MSE(gamma):   ", np.mean(vec_gamma_mse))
                    # print("Var(gamma):   ", np.mean(vec_gamma_var))
                    # print("CoD(mu):      ", np.mean(vec_mu_mse) / np.mean(vec_mu_var))
                    # print("CoD(gamma):   ", np.mean(vec_gamma_mse) / np.mean(vec_gamma_var))
