...
def fun_mu(X, j):
    p = len(X)
    # jp = np.mod(j + 2, p)  ## default
    jp = np.mod(j + 1, p)  ## setting 5
    # out = fun_sigm(X[j]) + .25 * X[jp]                     ## default
    # out = 2.0 * fun_sigm(10 * (out - 1.2)) ## (default + ) ## setting 1
    # out = 2.0 * fun_sigm(20 * (X[j] - 1.5)) + .25 * X[jp]  ## setting 2
    # out = 2.0 * fun_sigm(20 * (X[j] - 2.0)) + .25 * X[jp]  ## setting 3
    # out = 20 * fun_sigm(20 * (X[j] - 2.0)) + .25 * X[jp]   ## setting 4 
    # out = 0     ## setting null
    out = 20 * fun_sigm(50 * (X[jp] - 1.5))    ## setting 5 
    return out
...
def fun_gamma(X, j): 
    ...
    # jp = np.mod(j + 2, p) ## default 
    jp = np.mod(j + 1, p)   ## setting 5
    # out = X[j] + .25 * fun_sigm(X[jp]) ## default
    out = 20 * fun_sigm(50 * (X[j] - 1.5))   ## setting 5
    return out
        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                for j in range(K): 
                    ...
                    # mat_D[:, j] = (mat_D[:, j] > 1.0).astype(float)
                    # print(">>>> >>>> mean_d: ", np.mean(mat_D[:, j]).round(5))
                    # print(
                    #     "[qt0, qt5, qt25, qt50, qt75, qt95, qt100](mu(X_j)): ", 
                    ...
                    # )
                    # print(
                    #     "[qt0, qt5, qt25, qt50, qt75, qt95, qt100](D_j): ", 
                    #     ["{:.2f}".format(b) for b in np.quantile(
                    #         mat_D[:, j], 
                    #         [0.0, 0.05, 0.25, 0.50, 0.75, 0.95, 1.0]
                    #     )]
