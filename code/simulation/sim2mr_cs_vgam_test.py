        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            try:
                ...
                for j in range(K):
                    ...
                    mat_X_est = arr_X[np.mod(j+2, K)][idx_est, :]

                    print(">> mean_X_est: ", np.mean(mat_X_est, axis=0).round(3))

                    ts_X_est = torch.from_numpy(mat_X_est).float()
                    vec_den_est = model_den_est.pred(ts_X_est, bl_shift=True).detach().numpy()
                    vec_den_truth = scipy.stats.multivariate_normal.pdf(
                        mat_X_est, 
                        mean = mat_X_shift[j, :], 
                        cov = covmat_X
                    )
                    ...
                    # print(np.corrcoef(vec_den_truth, vec_den_est)[0, 1].round(5))

                plt.axis('equal')
                plt.axline((0, 0), slope=1, color='red', linestyle='--', alpha=0.8, label='y=x')

                fname_output = \
                    "output/den_est_cpr_shift_typecs" + str(type_cs) + str(ts.now().timestamp()) + ".png"
