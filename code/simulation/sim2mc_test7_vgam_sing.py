...
def main(argv): 
    ...
    vec_beta_cen_est_iter = np.zeros(n_iter)
    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            try:
                    ...
                    if den_est == "single":
                        ...
                        for j in range(K):
                            ...
                            mat_U_slope[:, j] = psi_u_inv_est * (
                                ...
                                np.power(vec_D_loc_diff, 2) - np.power(vec_D_cen - vec_D_cen_est, 2)
                            )
                ...
                vec_beta_cen_est_iter[i_iter] = vec_beta_est_cen[0]
                ...
                if path_out is None: 
                    print(
                        ...
                        ','.join(str(b.round(5)) for b in vec_beta_cen_est_iter),
                        sep=",",
                    )
                else:
                    print(
