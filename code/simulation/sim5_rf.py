        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                for j in range(K):
                    ...
                    mat_Z_X_nui = np.column_stack((vec_Z_nui, mat_X_nui))
                    ...
                    print("mean(Z): ", np.mean(vec_Z_nui))
                    print("mean(D): ", np.mean(vec_D_nui))
                    
                    print("FI_mu: ", model_mu.feature_importances_)

                    model_nu = RandomForestClassifier(n_estimators=n_rft)
                    model_nu.fit(mat_Z_X_nui, vec_D_nui)

                    print("FI_nu: ", model_nu.feature_importances_)

                    model_gamma = RandomForestRegressor(n_estimators=n_rft)
                    model_gamma.fit(mat_Z_X_nui, vec_Y_nui)

                    print("FI_gam: ", model_gamma.feature_importances_)

                    ## estimating beta
                    vec_mu_est = model_mu.predict_proba(mat_X_est)[:, 1]

                    vec_nu0_est = model_nu.predict_proba(mat_0_X_est)[:, 1]
                    vec_nu1_est = model_nu.predict_proba(mat_1_X_est)[:, 1]

                    vec_gam0_est = model_gamma.predict(mat_0_X_est)
                    vec_gam1_est = model_gamma.predict(mat_1_X_est)

                    print("diff(gam): ", np.mean(vec_gam1_est - vec_gam0_est))
