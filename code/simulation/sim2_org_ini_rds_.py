...
print(
    "rnd_np", "rnd_ds", "Average", "M1", "M2", "M3", "M4", "M5", 
    sep=","
)
...
#### randomization
for rnd_np in (178 + np.array(range(n_rnp))): 
    for rnd_ds in (140 + np.array(range(n_rds))):
        ...
        # for j_cen in [0]:
            ...
            for j in range(K):
                ...
                # print("density ratio: ", np.mean(mat_U_slope[:, j]))
                
                mat_U_slope[:, j] = mat_U_slope[:, j] * vec_D_cen * vec_D_loc_diff

                # print(" " * 40, "U_slope:       ", np.mean(mat_U_slope[:, j]))


            U_slope = np.mean(mat_U_slope)

            # print(" " * 40, "U_slope:       ", U_slope)
        ...
        ## final estimation
        print("vec_beta_est_cen: ", vec_beta_est_cen)
        ...
        print("local average: ", beta_est_ini)
