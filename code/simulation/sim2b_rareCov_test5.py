...
def main(argv): 
    ...
    prompt_help = [
        ...
        '--prob_d <probability of D=1>',
        '--df_t <probability of t distributio for u>',
    ...
    for rnd_np in (rnp_np_ini + np.array(range(n_rnp))): 
        for rnd_ds in (128 + np.array(range(n_rds))):
            ...
            try:
                ...
                # print("sd mat_u: ", np.std(mat_U).round(5))

                psi_u_cur = np.var(mat_U).round(5)
                psi_u_cur_inv = 1 / psi_u_cur
                ...
                mat_D = np.random.binomial(1, prob_d, size=(n, K))

                for j in range(K): 
                    # mat_D[:, j] = np.array(list(map(lambda X: fun_mu(X, j), arr_X[j]))) 
                    arr_X[j][:, 0] = arr_X[j][:, 0] + mat_D[:, j] - 0.3
                ...
                # print(">> mean_y: ", np.mean(mat_Y).round(5))
                # print(">> range_y: ", np.max(mat_Y), " - ", np.min(mat_Y))
                # print(">> q1 - q3: ", np.quantile(mat_Y, 0.25), " - ", np.quantile(mat_Y, 0.75))
                ...
                # beta_est_ini_ = beta_est_ini + 2 * np.random.randn()
                # beta_est_ini = beta_est_ini_
                # beta_est_ini = np.median(vec_beta_est_local)
                # beta_est_ini = np.max(vec_beta_est_local)
                ...
                for j_cen in range(K):
                    ...
                    if den_est == "single":
                        ...
                        for j in range(K):
                            ...
                            mat_U_slope[:, j] = np.exp(-.5 * psi_u_cur_inv * mat_U_slope[:, j]) ## density ratio
