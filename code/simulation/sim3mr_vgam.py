...
def main(argv): 
    ...
    prompt_help = [
        ...
        '--cor_dz <correlation between Z and D>',
        '--vgam <vgam setting>',
    ...
    if cor_dz < 0 or cor_dz > 1:
        sys.exit("cor_dz must be between 0 and 1.")
        ...
        for rnd_ds in (128 + np.array(range(n_rds))):
            try:
                ...
                for j in range(K): 
                    ...
                    vec_prob_d = mat_Z[:, j] * cor_dz + (1 - mat_Z[:, j]) * pz_cur / (1 - pz_cur) * (1 - cor_dz)

                    mat_D[:, j] = np.random.binomial(n = 1, p = vec_prob_d) ## rare treatment

                    mat_Y[:, j] = mat_D[:, j] * beta \
                        + np.array(list(map(lambda X: fun_gamma(X, j, mat_gam[:, j]), arr_X[j]))) + mat_U[:, j]

                # print(">> Pr(Z=1): ", np.mean(mat_Z).round(5))
                # print(">> Pr(D=1): ", np.mean(mat_D).round(5))
                # print(">> " * 5, ">> Pr(D == Z): ", np.mean(mat_D == mat_Z).round(5))
                ...
                for j in range(K): 
                    ...
                    vec_Z_ora_diff = vec_Z_est - np.array(list(map(lambda X: fun_mu(X, j, mu_alpha, mu_beta, mu_ip), mat_X_est))) 
                    vec_Y_ora_diff = vec_Y_est - np.array(list(map(lambda X: fun_gamma(X, j, mat_gam[:, j]), mat_X_est)))

                    vec_s_ora = (vec_Y_ora_diff - vec_D_est * beta_est_ini) * vec_Z_ora_diff
                ...
                for j_cen in range(K):
                    ...
                    if den_est == "single":
                        ...
                        vec_Z_cen_est = list_mu_est[j_cen].predict(mat_X_cen)
                        vec_Y_cen_diff = vec_Y_cen - list_gamma_est[j_cen].predict(mat_X_cen)
                        ...
                        for j in range(K):
                            ...
                            vec_Z_loc_est = list_mu_est[j].predict(mat_X_cen)
                            vec_Z_loc_diff = vec_Z_cen - vec_Z_loc_est
                            vec_Y_loc_diff = vec_Y_cen - list_gamma_est[j].predict(mat_X_cen)
                            ...
                            mat_U_slope[:, j] = mat_U_slope[:, j] \
                                * ((1 - vec_Z_cen) + (2 * vec_Z_cen - 1) * vec_Z_loc_est + 0.005) \
                                    / ((1 - vec_Z_cen) + (2 * vec_Z_cen - 1) * vec_Z_cen_est + 0.005)
